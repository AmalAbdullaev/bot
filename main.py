    ...
    try:
        if (message.text == "/start"):
            bot.send_message(message.chat.id, "Правила игры: Ты называешь город, потом"
                                              ...
                                              "закачиваются на Ь,Ъ,Ы   отправляем города на предыдущую букву."
                                              " В процессе игры города не должны повторяться."
                                              " Чтоб начать игру заново отправь /start"
                                              " Поехали, посмотрим на что ты способен  ")
            send.clear()
            send.append("!")
            file = open("city.txt",'r+',encoding = "windows-1251")
            cities = file.read()
